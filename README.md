...
Discord sports desk bot — **evidence-first daily analytics engine · force daily card with analysis**.
...
- **LOCK** when strict thresholds pass; otherwise **LEAN** (or **FORCED LEAN**)
- **Force daily picks**: when nothing clears the strict bar, promote best analyzed sides to FORCED LEAN (clearly labeled, smaller units) so the desk always posts a full card with analysis + reasoning
- Full analysis on every ticket: model %, edge, top-3 reasons, biggest risk, market signal, what-if, autopsy
...
- Historical tracker (`/logpick` · `/grade` · `/learn` · `/review`) — learns from record; history never rewritten
