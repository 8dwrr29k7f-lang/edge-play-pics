 ...
 * Force daily leans allowed (clearly labeled). Outcomes: STRONG PLAY / LEAN / FORCED LEAN / NO PLAY.
 */
...
export function standardizePick(p, { lotd = false, dataFreshness = null } = {}) {
  ...
  // Forced daily leans always render full analysis card (never collapse to NO PLAY)
  const noPlay = forced ? false : ev.playLevel === "NO PLAY";
  if (forced) {
  ...
  const finalBlock = noPlay
    ...
    : [
        ...
        `${ev.playEmoji} PLAY LEVEL: ${forced ? "LEAN ⚡ FORCED" : ev.playLevel}`,
        "━━━━━━━━━━━━━━━━",
        "",
        "🔥 TOP 3 REASONS",
        `• ${ev.top3[0]}`,
        ...
        "",
        "FINAL:",
        `${ev.playEmoji} ${forced ? "LEAN ⚡ FORCED" : ev.playLevel} on ${selection} at ${ev.oddsDisplay} (model ${ev.probabilityPct}%, edge ${ev.edge != null ? (ev.edge >= 0 ? "+" : "") + ev.edge + "%" : "n/a"}). ${forced ? "Forced daily lean with full analysis." : "Survived case-for + case-against."}`
      ]
        .filter((x) => x != null)
        .join("\n");
...
function formatNoPlay(reason) {
  return [
    ...
    "• Evidence below bar (non-forced path)",
    "",
    "⚠️ BIGGEST RISK",
    `• ${reason}`,
    "",
