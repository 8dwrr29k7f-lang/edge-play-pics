...
if [ ! -f src/index.js ] || [ ! -f src/data/desk.js ] || [ ! -f src/cashout.js ]; then
...
# Ensure health listener (Railway)
if ! grep -q "Health listener" src/index.js 2>/dev/null; then
  echo "Injecting health listener..."
  node --input-type=module -e '
import fs from "fs";
let t = fs.readFileSync("src/index.js", "utf8");
if (!t.includes("import http")) t = t.replace("from \"discord.js\";", "from \"discord.js\";\nimport http from \"http\";");
if (!t.includes("Health listener")) {
  t = t.replace("assertConfig();\n\nconst client = new Client({",
`assertConfig();
const PORT = Number(process.env.PORT) || 3000;
http.createServer((req,res)=>{res.writeHead(200,{"Content-Type":"text/plain"});res.end("EDGE PLAY PICS · alive\\n");}).listen(PORT,()=>console.log("Health listener on :"+PORT));
const client = new Client({`);
}
if (!t.includes("FATAL Discord login failed")) {
  t = t.replace("client.login(config.token);",
`client.login(config.token).catch(e=>{console.error("FATAL Discord login failed:",e.message);process.exit(1);});`);
}
fs.writeFileSync("src/index.js", t);
console.log("Health injected");
' || true
fi
